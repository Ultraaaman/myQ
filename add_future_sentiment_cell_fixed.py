...
if factor_results is not None:
    ...
    try:
        ...
        if os.path.exists(sentiment_file):
            ...
            # 检查factor_results是否已经有date列，如果没有则创建
            if 'date' not in factor_results.columns:
            ...
            # 准备合并的数据
            factor_results_for_merge = factor_results.copy()

            # 如果index不是date列，重置索引
            if factor_results_for_merge.index.name != 'date':
                factor_results_for_merge = factor_results_for_merge.reset_index()
                # 如果重置索引后没有date列，创建它
                if 'date' not in factor_results_for_merge.columns:
                    factor_results_for_merge['date'] = factor_results_for_merge.index.date

            # 合并数据
            factor_results_with_sentiment = pd.merge(
                factor_results_for_merge,
                daily_sentiment,
                on='date',
                how='left'
            )
            ...
            # 重新设置索引
            if 'date' in factor_results_with_sentiment.columns:
                factor_results_with_sentiment = factor_results_with_sentiment.set_index('date')
        ...
        interaction_factor_columns = [col for col in factor_results.columns if any(word in col for word in
                                     ['interaction', 'filtered', 'comprehensive', 'certainty_vol', 'volume_ratio', 'volume_change'])]
        ...
        # 显示合并后的数据预览
        print(f"\n📋 扩展后数据预览:")
        all_factor_cols = future_return_columns + sentiment_factor_columns[:5] + technical_factor_columns[:5]
        if len(all_factor_cols) > 0:
            print("最新5天数据 (部分列):")
            display(factor_results[all_factor_cols[:10]].tail())
